...
export const messagesApi = {
  list: (conversationId: string) => api.get<MessageDTO[]>(`/messages/${conversationId}`),

  sendText: (conversationId: string, payload: { content: string }) =>
    api.post<MessageDTO>(`/messages/${conversationId}`, payload),
  ...
  sendAttachment: async (
    ...
    payload: { file: File; attachmentType: NonNullable<MessageDTO["attachmentType"]>; content?: string },
  ) => {
    ...
    return axiosInstance.post<MessageDTO>(`/messages/${conversationId}`, form, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },

  edit: (id: string, payload: { content: string }) => api.patch<MessageDTO>(`/messages/${id}`, payload),
